...
private fun CustomerPickerDialog(
    ...
    // After create succeeds, auto-select the new customer
    LaunchedEffect(uiState.operationSuccess) {
        if (uiState.operationSuccess) {
            val created = uiState.customers.firstOrNull {
                it.name.equals(newName, ignoreCase = true) && it.phone == newPhone
            }
            if (created != null) onCustomerSelected(created.id, created.name, created.phone)
            else onDismiss()
        }
    }
                        ...
                        LazyColumn(modifier = Modifier.height(260.dp)) {
                            items(uiState.customers.take(20)) { customer ->
                                ListItem(
                                    ...
                                    modifier = Modifier.clickable { onCustomerSelected(customer.id, customer.name, customer.phone) }
                                )
                                HorizontalDivider()
                            }
                        }
