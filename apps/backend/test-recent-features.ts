...
async function runTest() {
  try {
    ...
    const currentSub = await axios.get(
      `${BASE_URL}/billing/subscription/current?module=MOBILE_SHOP`,
      { headers },
    );
    console.log('Current Plan:', currentSub.data.current.name);
    console.log(
      'Current Cycle:',
      currentSub.data.current.priceSnapshot ? 'PAID' : 'TRIAL',
    );

    // 2. USAGE HISTORY
    console.log('\n--- 2. Testing Usage History ---');
    const history = await axios.get(`${BASE_URL}/tenant/usage-history?days=7`, {
      headers,
    });
    console.log(`History Entries: ${history.data.length}`);
    if (!Array.isArray(history.data))
      throw new Error('Usage history is not an array');
    ...
    const plansValues = await axios
      .get(`${BASE_URL}/plans?module=MOBILE_SHOP`, { headers })
      .catch(() => null);
    ...
    if (plansValues && plansValues.data.length > 0) {
      ...
      console.log(
        `Checking downgrade to: ${plansValues.data[0].name} (${targetPlanId})`,
      );
    } else {
    ...
    try {
      ...
      console.log(
        'Downgrade check failed (expected if plan invalid):',
    ...
    if (currentSub.data.current.planId) {
      // If has active plan
      try {
