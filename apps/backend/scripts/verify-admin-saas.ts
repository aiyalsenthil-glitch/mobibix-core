...
async function verifyAdminFeatures() {
  ...
  try {
    ...
    if (
      lookupResult.user.email !== testEmail ||
      lookupResult.tenants.length !== 1
    ) {
    ...
    const tenantAfterEnable = await prisma.tenant.findUnique({
      where: { id: tenantId },
    });
    ...
    console.log(
      ...
      tenantAfterEnable?.whatsappCrmEnabled,
    );
    ...
    const tenantAfterDisable = await prisma.tenant.findUnique({
      where: { id: tenantId },
    });
    ...
    console.log(
      'Tenant WhatsApp Enabled:',
      tenantAfterDisable?.whatsappCrmEnabled,
    );
    console.log('CRM Addon Status:', crmSubDisabled?.status);

    if (
      tenantAfterDisable?.whatsappCrmEnabled ||
      crmSubDisabled?.status !== 'CANCELLED'
    ) {
    ...
    const AdminAnalyticsController =
      require('../src/core/admin/analytics/admin-analytics.controller').AdminAnalyticsController;
    ...
    const AdminTenantController =
      require('../src/core/admin/tenant/admin-tenant.controller').AdminTenantController;
    ...
    try {
      ...
      const tenants = await adminTenantController.listTenants(
        'Verify',
        '1',
        '10',
      );
      ...
      if (tenants.data.length === 0)
        console.warn('⚠️ No tenants found matching "Verify"');

      if (tenants.data.length > 0) {
        const impersonation = await adminTenantController.impersonateTenant(
          tenants.data[0].id,
        );
        console.log(
          'Impersonation Token:',
          impersonation.accessToken ? 'Generated' : 'Missing',
        );
      }
    ...
    // =================================================================
    console.log('\n🧪 TEST 7: MDM - Global Products');
    const AdminMdmController =
      require('../src/core/admin/mdm/admin-mdm.controller').AdminMdmController;
