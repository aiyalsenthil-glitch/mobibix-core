...
function mapLegacyRoleToSystemRole(
  legacyRole: string,
...
async function migrateLegacyRoles() {
  ...
  if (
    ...
    !systemRoles['GYM_MANAGER']
  ) {
    throw new Error(
      'System Roles not found! Did you run the seed script first?',
    );
  }
  ...
  for (const staff of shopStaff) {
    ...
    const moduleType = enabledModules.includes(ModuleType.GYM)
      ? ModuleType.GYM
      : ModuleType.MOBILE_SHOP;

    const mappedSystemRoleName = mapLegacyRoleToSystemRole(
      staff.role,
      moduleType,
    );
    ...
    if (!mappedRoleId) {
      console.warn(
        `⚠️ Warning: Could not find system role ID for ${mappedSystemRoleName} (Legacy: ${staff.role})`,
      );
      continue;
    }
  ...
  console.log(
    `✅ Migrated ${shopStaffMigrated} ShopStaff records to dynamic Role IDs.`,
  );
  ...
  console.log(
    `✅ Migrated ${invitesMigrated} StaffInvite records to dynamic Role IDs.`,
  );

  console.log('\n🎉 Legacy Role Data Migration Complete! 🎉');
  console.log(
    'You can now safely drop the legacy `role` columns in Phase 2/3.',
  );
}
