...
async function testMixedPayment() {
  ...
  for (const pm of paymentMethods) {
    if (pm.mode !== 'CREDIT') {
      ...
      console.log(
        `Payment: ${pm.mode}, Amount: ${pm.amount}, Paisa: ${amountPaisa}`,
      );
    }
  }
  ...
  const invoiceStatus =
    totalPaidPaisa >= totalAmountPaisa - 100 ? 'PAID' : 'PARTIALLY_PAID';
  ...
  console.log(
    `Invoice Total 2 (Paisa): ${totalAmountPaisa2} (Expecting 11853)`,
  );

  const paymentMethods2 = [
    { mode: 'CASH', amount: 50.53 },
    { mode: 'UPI', amount: 68.0 },
  ]; // Total 118.53
  ...
  const invoiceStatus2 =
    totalPaidPaisa2 >= totalAmountPaisa2 - 100 ? 'PAID' : 'PARTIALLY_PAID';
