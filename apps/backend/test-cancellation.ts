...
async function runTest() {
  try {
    ...
    if (!userTenant)
      throw new Error('UserTenant not found for token generation');
    ...
    if (productAfterAdd.data.stockQty !== 8) {
      throw new Error(
        `Stock deduction failed! Expected 8, got ${productAfterAdd.data.stockQty}`,
      );
    }
    ...
    try {
      ...
      console.log(
        '✅ Success: Cancellation blocked as expected.',
    ...
    if (productAfterCancel.data.stockQty !== 10) {
      throw new Error(
        `Stock restoration failed! Expected 10, got ${productAfterCancel.data.stockQty}`,
      );
    }
