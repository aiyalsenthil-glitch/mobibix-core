...
async function bootstrap() {
  ...
  try {
    let tenant = await prisma.tenant.findFirst({
      where: { name: 'JobCard Test Tenant' },
    });
    if (!tenant) {
    ...
    const readyJob = await jobService.updateStatus(
      mockUser,
      shop.id,
      createdJob.id,
      'READY',
    ...
    const jobWithParts = await prisma.jobCard.findFirst({
      where: { id: createdJob.id },
      include: { shop: true, parts: { include: { product: true } } },
    });
    ...
    if (
      ...
      invoice.items[0].product.name === 'Repair Service'
    ) {
      console.log(
        `✅ Invoice successfully generated: ${invoice.invoiceNumber}`,
      );
      console.log(
        `✅ Service Line Item Amount: ₹${invoice.items[0].lineTotal / 100}`,
      );
      if (invoice.items[0].lineTotal === 150000) {
        // 1500 * 100 paisa
        console.log(
          `🟢 PASS: Explicit Labor Charge perfectly maps to Invoice Total`,
        );
      } else {
        console.error(
          `🔴 FAIL: Invoice Total mismatch. Expected 150000, got ${invoice.items[0].lineTotal}`,
        );
      }
    } else {
    ...
    const deliveredJob = await jobService.updateStatus(
      mockUser,
      shop.id,
      createdJob.id,
      'DELIVERED',
    ...
    if (deliveredJob.deliveredAt) {
      ...
      console.log(
        `🟢 PASS: deliveredAt timestamp explicitly minted -> ${deliveredJob.deliveredAt}`,
      );
    } else {
      console.error(
        `🔴 FAIL: deliveredAt is null after DELIVERED status mapping.`,
      );
    }
    ...
    const scrappedJob = await jobService.updateStatus(
      mockUser,
      shop.id,
      scrapJob.id,
      'SCRAPPED',
    ...
    if (scrappedJob.scrappedAt) {
      ...
      console.log(
        `🟢 PASS: scrappedAt timestamp explicitly minted -> ${scrappedJob.scrappedAt}`,
      );
    } else {
      console.error(
        `🔴 FAIL: scrappedAt is null after SCRAPPED status mapping.`,
      );
    }
