...
async function testRefresh() {
  try {
    ...
    try {
        ...
        console.log('1. Empty body check:', e.response?.status === 400 || e.response?.status === 401 ? 'PASS' : 'FAIL', e.response?.data);
    }

    try {
        await axios.post(`${API_URL}/auth/refresh`, { refreshToken: 'invalid-token' });
    } catch (e: any) {
        console.log('2. Invalid token check:', e.response?.status === 401 ? 'PASS' : 'FAIL', e.response?.data);
    }
    
    console.log('To fully test, we need a valid login. Skipping full integration test for now.');
