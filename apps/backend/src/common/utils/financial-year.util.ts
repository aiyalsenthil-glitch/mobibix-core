 ...
 *
 * ⚠️ STANDARDIZED: Matches frontend utility structure
 */
 ...
 * @param date - Date to calculate financial year for (defaults to current date)
 * @returns Financial year in format "YYZZ" (e.g., "2526")
 */
 ...
 * @param date - Date to calculate financial year for (defaults to current date)
 * @returns End date of the financial year (March 31st, 23:59:59)
 */
export function getFinancialYearEnd(date: Date = new Date()): Date {
  ...
  if (month < 4) {
    // Before April - FY ends this year (March 31)
    return new Date(year, 2, 31, 23, 59, 59); // Month 2 = March (0-indexed)
  } else {
    // April or later - FY ends next year
    return new Date(year + 1, 2, 31, 23, 59, 59);
  }
}
 ...
 */
export function isInFinancialYear(date: Date, financialYear: string): boolean {
