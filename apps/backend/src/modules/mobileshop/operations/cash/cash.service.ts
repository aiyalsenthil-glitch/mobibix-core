...
export class CashService {
  ...
  async getDailySummary(tenantId: string, shopId: string, date: string) {
    ...
    for (const fe of financialEntries) {
      ...
      if (
        fe.referenceType === FinanceRefType.INVOICE ||
        (fe.referenceType === FinanceRefType.RECEIPT &&
          fe.type === FinanceType.IN) ||
        ...
        fe.referenceType === FinanceRefType.EXPENSE ||
        fe.referenceType === FinanceRefType.SALARY
      ) {
        if (isCash) expenseCash += amount;
      } else if (
        fe.referenceType === FinanceRefType.PURCHASE ||
        (fe.referenceType === FinanceRefType.RECEIPT &&
          fe.type === FinanceType.OUT)
      ) {
        ...
        if (isCash) {
          if (fe.type === FinanceType.IN) {
            if (note.includes('withdraw') || note.includes('bank to cash'))
              cashWithdrawFromBank += amount;
            else otherCashIn += amount;
          } else {
            if (note.includes('deposit') || note.includes('cash to bank'))
              cashDepositToBank += amount;
    ...
    for (const v of vouchers) {
      ...
      if (v.paymentMethod === PaymentMode.CASH) {
        if (
          ...
          v.voucherType === VoucherType.SALARY
        )
          expenseCash += v.amount;
        else if (v.voucherType === VoucherType.SUPPLIER)
          supplierPaymentsCash += v.amount;
      }
    }
    ...
    for (const sp of supplierPayments) {
      ...
      if (sp.paymentMethod === PaymentMode.CASH)
        supplierPaymentsCash += sp.amount;
    }

    const totalIn = salesCash + cashWithdrawFromBank + otherCashIn;
    const totalOut =
      cashDepositToBank + supplierPaymentsCash + expenseCash + otherCashOut;
    ...
    return {
      ...
      shiftInfo:
        shop.cashClosingMode === CashClosingMode.SHIFT_AND_DAILY
          ? {
              totalShifts: currentShifts.length,
              closedShifts: currentShifts.filter(
                (s) => s.status === ShiftClosingStatus.CLOSED,
              ).length,
  ...
  async dailyClose(tenantId: string, userId: string, dto: DailyCloseDto) {
    ...
    const existing = await this.prisma.dailyClosing.findUnique({
      where: {
        tenantId_shopId_date: { tenantId, shopId: dto.shopId, date: dayDate },
      },
    });

    if (existing?.status === DailyClosingStatus.SUBMITTED) {
      throw new ConflictException(
        `Day ${dto.date} is already submitted and locked.`,
      );
    }
    ...
    const sc =
      isManual && dto.manualEntries?.salesCash !== undefined
        ? dto.manualEntries.salesCash
        : summary.salesCash;
    const su =
      isManual && dto.manualEntries?.salesUpi !== undefined
        ? dto.manualEntries.salesUpi
        : summary.salesUpi;
    const sCard =
      isManual && dto.manualEntries?.salesCard !== undefined
        ? dto.manualEntries.salesCard
        : summary.salesCard;
    const sb =
      isManual && dto.manualEntries?.salesBank !== undefined
        ? dto.manualEntries.salesBank
        : summary.salesBank;
    const cw =
      isManual && dto.manualEntries?.cashWithdrawFromBank !== undefined
        ? dto.manualEntries.cashWithdrawFromBank
        : summary.cashWithdrawFromBank;
    const cd =
      isManual && dto.manualEntries?.cashDepositToBank !== undefined
        ? dto.manualEntries.cashDepositToBank
        : summary.cashDepositToBank;
    const sp =
      isManual && dto.manualEntries?.supplierPaymentsCash !== undefined
        ? dto.manualEntries.supplierPaymentsCash
        : summary.supplierPaymentsCash;
    const ec =
      isManual && dto.manualEntries?.expenseCash !== undefined
        ? dto.manualEntries.expenseCash
        : summary.expenseCash;
    const oi =
      isManual && dto.manualEntries?.otherCashIn !== undefined
        ? dto.manualEntries.otherCashIn
        : summary.otherCashIn;
    const oo =
      isManual && dto.manualEntries?.otherCashOut !== undefined
        ? dto.manualEntries.otherCashOut
        : summary.otherCashOut;

    let expectedClosingCash = summary.expectedClosingCash;
    if (isManual) {
      expectedClosingCash =
        summary.openingCash + sc + cw + oi - ec - sp - oo - cd;
    }
  ...
  async openShift(tenantId: string, userId: string, dto: ShiftOpenDto) {
    ...
    if (shop.cashClosingMode !== CashClosingMode.SHIFT_AND_DAILY) {
      throw new BadRequestException(
        'Shift closing is not enabled for this shop.',
      );
    }
    ...
    if (openShift)
      throw new ConflictException('A shift is already open for this shop.');
  ...
  async closeShift(tenantId: string, userId: string, dto: ShiftCloseDto) {
    ...
    if (!openShift)
      throw new NotFoundException('No open shift found for this shop.');
  ...
  async getMonthlyProfit(
    ...
    const totalCogs =
      cogs.reduce(
        (acc, entry) => acc + entry.quantity * (entry.costPerUnit ?? 0),
        0,
      ) / 100;
    ...
    const netProfit =
      totalSales - totalCogs - totalExpenses - totalRefunds - totalInvLoss;
