...
async function bootstrap() {
  ...
  try {
      ...
      const mockRes = {
        status: (code) => ({ json: (body) => console.log(`Response: ${code}`, body), send: (txt) => console.log(`Response: ${code}`, txt) }),
      };
      ...
      if (process.env.WHATSAPP_APP_SECRET) {
          console.log('⚠️ Signature check enabled. Mock webhook might fail 403. Check logs.');
      }
      ...
      if (log) {
          ...
          console.log('⚠️ No log found. Signature check probably blocked it or routing failed.');
      }
