...
async function verify() {
  ...
  try {
    ...
    if (invoice) {
        ...
        const result = await resolver.resolveVariables(['invoiceLink', 'invoice_link'], context);
    ...
    if (jobCard) {
        ...
        const result = await resolver.resolveVariables(['jobTrackingLink', 'job_tracking_link'], context);
