...
async function verify() {
  ...
  try {
    ...
    logger.log(
      `Sales Invoice Created: ${invoice.invoiceNumber}, Total: ${invoice.totalAmount}`,
    );
    ...
    if (stock !== 8)
      throw new Error(`Stock mismatch after sale. Expected 8, got ${stock}`);
  ...
  try {
    ...
    await repairService.stockOutForRepair(tenant.id, {
      ...
      jobCardId: job.id,
      items: [
        { shopProductId: spareProduct.id, quantity: 1, costPerUnit: 500 },
      ],
    } as any);
    ...
    if (stockAfterRepairOut !== 7)
      throw new Error(
        `Stock mismatch after repair out. Expected 7, got ${stockAfterRepairOut}`,
      );
    ...
    const bill = await repairService.generateRepairBill(tenant.id, {
      ...
      pricesIncludeTax: false,
      services: [{ description: 'Screen Replacement Labor', amount: 500 }],
      parts: [
        {
          shopProductId: spareProduct.id,
          quantity: 1,
    ...
    logger.log(
      `Repair Bill Generated: ${bill.invoiceNumber}, Total: ${bill.totalAmount}`,
    );

    // Verify Job Status
    const updatedJob = await prisma.jobCard.findUnique({
      where: { id: job.id },
    });
    if (updatedJob?.status !== 'DELIVERED')
      throw new Error('Job status is not DELIVERED');
