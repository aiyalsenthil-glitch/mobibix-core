...
async function verifyAdminSuite() {
  ...
  try {
    ...
    const statsRes = await axios.get(`${API_URL}/admin/analytics/global`, { headers });
    console.log('✅ Global stats:', statsRes.data);
    
    if (typeof statsRes.data.mrr !== 'number' || typeof statsRes.data.totalTenants !== 'number') {
       console.error('❌ Stats structure invalid!');
    }

    // 2. Verify HSN MDM
    console.log('🚀 Testing POST /admin/mdm/hsn...');
    const hsnRes = await axios.post(`${API_URL}/admin/mdm/hsn`, {
      code: 'TESTHSN',
    ...
    const hsnList = await axios.get(`${API_URL}/admin/mdm/hsn?search=TESTHSN`, { headers });
    ...
    const productRes = await axios.post(`${API_URL}/admin/mdm/products`, {
    ...
    const productList = await axios.get(`${API_URL}/admin/mdm/products?search=Test`, { headers });
    console.log('✅ Product list sample:', productList.data.data[0]);
    
    if (productList.data.data.some((p: any) => p.name === 'Test Global Product')) {
