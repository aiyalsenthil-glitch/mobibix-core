...
export class RazorpayWebhookController {
  ...
  async handleWebhook(
    ...
    const rawSecrets = this.configService.get<string>('RAZORPAY_WEBHOOK_SECRET');
    if (!rawSecrets) {
    ...
    const secrets = rawSecrets.split(',').map(s => s.trim()).filter(s => s.length > 0);
    
    let isValid = false;
    const bodyBuffer = req.rawBody ?? Buffer.from(JSON.stringify(body));

    for (const secret of secrets) {
      if (this.REMOVED_PAYMENT_INFRAService.validateWebhookSignature(bodyBuffer, signature, secret)) {
        isValid = true;
        break;
      }
    }
