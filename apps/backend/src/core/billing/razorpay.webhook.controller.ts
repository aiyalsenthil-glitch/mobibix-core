...
import { SubscriptionStatus, PaymentStatus, AutopayStatus, BillingType } from '@prisma/client';
...
export class RazorpayWebhookController {
    ...
    try {
      switch (event) {
        ...
        case 'subscription.cancelled':
          await this.handleSubscriptionCancelled(body.payload.subscription.entity);
          break;
  ...
  private async handleSubscriptionActivated(subEntity: any) {
    ...
    if (subscription) {
        ...
        this.logger.log(`✅ TenantSubscription ${subscription.id} activated (AutoPay).`);
    } else {
  ...
  private async handleSubscriptionCharged(subEntity: any, paymentEntity: any) {
      const subId = subEntity.id;
      this.logger.log(`Subscription Charged: ${subId}, Payment: ${paymentEntity.id}`);
      ...
      if (subscription) {
          ...
          this.logger.log(`✅ TenantSubscription ${subscription.id} renewed (AutoPay).`);
      }
  }
