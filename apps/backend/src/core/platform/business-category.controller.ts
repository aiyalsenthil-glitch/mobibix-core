...
export class BusinessCategoryController {
  ...
  @Roles(UserRole.SUPER_ADMIN, UserRole.ADMIN)
  @Post()
  async create(@Body() data: { name: string; description?: string; isComingSoon?: boolean; sortOrder?: number }) {
    return this.businessCategoryService.create(data);
  }

  @Roles(UserRole.SUPER_ADMIN, UserRole.ADMIN)
  @Patch(':id')
  async update(@Param('id') id: string, @Body() data: { name?: string; description?: string; isComingSoon?: boolean; isActive?: boolean; sortOrder?: number }) {
