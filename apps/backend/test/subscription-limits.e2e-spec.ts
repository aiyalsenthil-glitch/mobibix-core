...
describe('Subscription Limits & Downgrade (e2e)', () => {
  ...
  const createTenant = async () => {
    const mobile = `98${Math.floor(Math.random() * 100000000).toString().padStart(8, '0')}`;
  ...
  describe('Downgrade Pre-check', () => {
    ...
    afterAll(async () => {
       ...
       await prisma.tenantSubscription.deleteMany({ where: { tenantId: dTenantId }});
       ...
       await prisma.plan.deleteMany({ where: { code: { in: ['MOBIBIX_STANDARD_LIMIT_TEST', 'MOBIBIX_PRO_LIMIT_TEST']}}});
    });
    ...
    it('should BLOCK downgrade if staff limit exceeded', async () => {
      ...
      const staffUsers = [];
      for(let i=0; i<3; i++) {
        const u = await prisma.user.create({ data: { mobile: `900000000${i}` }});
        await prisma.userTenant.create({
      ...
      // Check count
      const count = await prisma.userTenant.count({ where: { tenantId: dTenantId, role: UserRole.STAFF }});
      expect(count).toBe(3);
      ...
      // Cleanup Staff
      await prisma.userTenant.deleteMany({ where: { tenantId: dTenantId, role: UserRole.STAFF }});
  ...
  describe('TRIAL Daily Limit', () => {
      ...
      afterAll(async () => {
         ...
         await prisma.whatsAppPhoneNumber.deleteMany({ where: { tenantId: tTenantId }});
         await prisma.whatsAppSetting.deleteMany({ where: { tenantId: tTenantId }});
         await prisma.whatsAppTemplate.deleteMany({ where: { id: templateId }});
         await prisma.tenantSubscription.deleteMany({ where: { tenantId: tTenantId }});
