...
describe('WhatsApp Integration (e2e)', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      const standardPlan = await prisma.plan.findFirst({ where: { code: 'MOBIBIX_STANDARD' } });
  ...
  afterAll(async () => {
    ...
    await prisma.subscriptionAddon.deleteMany({ where: { subscription: { tenantId } } }); // Cascade usually handles this but safety first
    ...
    await prisma.whatsAppTemplate.deleteMany({ where: { templateKey: 'TEST_ALERT' } });
  ...
  it('STANDARD Plan: Should BLOCK Premium Feature (Automation)', async () => {
    ...
    const template = await prisma.whatsAppTemplate.findFirst({ where: { templateKey: 'TEST_ALERT' } });
    ...
    expect(log.error).toContain('Plan missing feature: WHATSAPP_ALERTS_AUTOMATION');
  });

  it('PRO Plan: Should ALLOW Premium Feature', async () => {
    // Upgrade to PRO
    const proPlan = await prisma.plan.findFirst({ where: { code: 'MOBIBIX_PRO' } });
  ...
  it('STANDARD + Addon: Should ALLOW Premium Feature', async () => {
    ...
    // Add Addon
    const addonPlan = await prisma.plan.findFirst({ where: { code: 'WHATSAPP_CRM' } });
    if (!addonPlan) throw new Error('Addon Plan not found');
    const subscription = await prisma.tenantSubscription.findFirst({ where: { tenantId } });
