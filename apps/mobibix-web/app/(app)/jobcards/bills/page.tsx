...
const STATUS_COLORS: Record<InvoiceStatus, string> = {
  ...
  CREDIT: "bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-400",
...
export default function JobCardBillsPage() {
  ...
  const {
    ...
    useCallback(async () => {
      ...
      return await listInvoices(selectedShopId, true);
    }, [selectedShopId]),
    [selectedShopId],
    [] as SalesInvoice[],
  );
  ...
  return (
    <div>
          ...
          <div className={`${theme === "dark" ? "bg-white/5 border-white/10" : "bg-white border-gray-200"} border rounded-lg p-4 mb-6 shadow-sm`}>
            <div className="flex items-center gap-4">
              <div className="flex-1">
                <label className={`block text-sm font-medium mb-2 ${theme === "dark" ? "text-stone-300" : "text-black"}`}>
                  Select Shop
                </label>
                <select
                  ...
                  className={`w-full px-4 py-2 rounded-lg font-medium focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500 border ${
                    theme === "dark" ? "bg-stone-900 border-white/20 text-white" : "bg-white border-gray-300 text-black"
                  }`}
                >
                  <option value="" className={theme === "dark" ? "bg-stone-900 text-white" : "bg-white text-black"}>
                    -- Select a shop --
                  </option>
                  {shops.map((shop) => (
                    <option key={shop.id} value={shop.id} className={theme === "dark" ? "bg-stone-900 text-white" : "bg-white text-black"}>
                      {shop.name}
                    </option>
                  ))}
                </select>
      ...
      {error && (
        <div className={`border px-4 py-3 rounded-lg mb-6 ${theme === "dark" ? "bg-red-500/15 border-red-500/40 text-red-300" : "bg-red-50 border-red-200 text-red-700"}`}>
          {displayError}
        </div>
      )}
      ...
      {!selectedShopId ? (
        <div className="text-center py-12">
          <p className={`mb-4 ${theme === "dark" ? "text-stone-400" : "text-zinc-600"}`}>
            Please select a shop to view job card bills
          </p>
        </div>
      ) : isLoading ? (
        <div className={`text-center py-12 ${theme === "dark" ? "text-stone-400" : "text-zinc-600"}`}>
          Loading bills...
        </div>
      ) : invoices.length === 0 ? (
        <div className="text-center py-12">
        ...
        <div className={`border rounded-lg overflow-hidden shadow-sm ${theme === "dark" ? "bg-white/5 border-white/10" : "bg-white border-gray-200"}`}>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className={`border-b ${theme === "dark" ? "bg-white/5 border-white/10" : "bg-gray-50 border-gray-200"}`}>
                <tr>
                  {["Invoice #", "Customer", "Amount", "Payment", "Status", "Date", "Actions"].map((header) => (
                    <th key={header} className={`text-left px-4 py-3 text-sm font-semibold ${theme === "dark" ? "text-stone-300" : "text-black"}`}>
                      {header}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className={`divide-y ${theme === "dark" ? "divide-white/10" : "divide-gray-200"}`}>
                {invoices.map((inv) => {
                  ...
                  const rowBgClass = hasBalance
                    ? theme === "dark" ? "bg-amber-500/5 hover:bg-amber-500/10" : "bg-amber-50 hover:bg-amber-100/50"
                  ...
                  return (
                    <tr key={inv.id} className={`transition ${rowBgClass}`}>
                      ...
                      <td className={`px-4 py-3 text-sm ${theme === "dark" ? "text-white" : "text-black"}`}>
                        <div className="space-y-1">
                          ...
                          {inv.balanceAmount !== undefined && (
                            <div className={`text-xs font-semibold ${inv.balanceAmount > 0 ? (theme === "dark" ? "text-red-400" : "text-red-600") : (theme === "dark" ? "text-green-400" : "text-green-600")}`}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${PAYMENT_BADGES[inv.paymentMode]}`}>
                          {inv.paymentMode}
                        </span>
                      </td>
                      <td className="px-4 py-3">
