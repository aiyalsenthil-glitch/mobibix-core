...
export function mapInvoiceToPrintData(ctx: AdapterContext): PrintDocumentData {
  ...
  const isInterState =
    shop.state && invoice.customerState && shop.state !== invoice.customerState;
  ...
  const items: PrintLineItem[] = (invoice.items || []).map((item) => {
    ...
    const taxableValue =
      item.taxableValue ?? (item.lineTotal || 0) - (item.gstAmount || 0);
  ...
  return {
    ...
    meta: {
      ...
      "Financial Year": getFinancialYear(new Date(invoice.invoiceDate)),
      ...
      ...(invoice.jobCard
        ? {
            ...
            ...(invoice.jobCard.deviceSerial
              ? { "Serial/IMEI": invoice.jobCard.deviceSerial }
              : {}),
          }
        : {}),
