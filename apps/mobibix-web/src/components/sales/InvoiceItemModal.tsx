...
export function InvoiceItemModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const effectiveRate = createMode
        ? typeof newSalePrice === "number"
          ? newSalePrice
          : parseFloat(newSalePrice || "0")
        : rate;
      ...
      if (pricesIncludeTax) {
        ...
        finalGstAmount =
          Math.round(((actualBase * effectiveGstRate) / 100) * 100) / 100;
      }

      const item: InvoiceItem = {
        shopProductId: finalProduct.id,
        quantity,
        rate: pricesIncludeTax
          ? effectiveRate / (1 + effectiveGstRate / 100)
          : effectiveRate,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-stone-900 border border-slate-200 dark:border-white/10 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <>
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                <div className="relative">
                  <input
                    type="text"
                    className="w-full px-4 py-2 border border-slate-200 dark:border-white/10 rounded-lg bg-slate-50 dark:bg-black/20 text-slate-900 dark:text-white focus:ring-2 focus:ring-teal-500 outline-none"
                  ...
                  <div className="w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg max-h-60 overflow-auto z-10">
                    ...
                    {filteredProducts.map((p) => (
                      ...
                      >
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded ${(p.stockQty ?? 0) > 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                        >
                          Stock: {p.stockQty ?? 0}
                        </span>
                      </button>
